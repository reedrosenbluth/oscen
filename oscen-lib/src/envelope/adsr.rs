...
impl SignalProcessor for AdsrEnvelope {
    ...
    ///
    /// AdsrEnvelopeIO contains { gate: &'io [EventInstance], output: f32 }
    fn process<'a>(&mut self, _sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        ...
        // Note: AdsrEnvelopeIO has lifetime parameter because of event input
        let mut io = AdsrEnvelopeIO {
            gate: self.events_gate(context),
            output: 0.0,
        };

        // Process gate events from IO struct
        for event in io.gate.iter() {
        ...
        // Write to IO struct output
        io.output = self.level;
