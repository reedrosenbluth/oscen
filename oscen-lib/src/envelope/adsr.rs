...
pub struct AdsrEnvelope {
    #[input(event, handler = "handle_gate")]
    gate: (),
...
impl AdsrEnvelope {
    ...
    fn handle_gate(&mut self, event: &EventInstance) {
    ...
    pub fn process(&mut self, _sample_rate: f32) -> f32 {
        // Macro already populated attack, decay, sustain, release values
        // Apply parameter constraints and update cached values
        self.apply_parameters(self.attack, self.decay, self.sustain, self.release);

        // Process the envelope stage
        self.process_stage();
...
// SignalProcessor is now FULLY auto-generated by the macro!
// The macro:
// 1. Auto-populates value inputs (attack, decay, sustain, release)
// 2. Calls handle_gate() for each gate event
// 3. Calls our process() method
//
// TODO: Need to handle init() and is_active() differently
// For now, we'll handle sample_rate updates in process() itself
