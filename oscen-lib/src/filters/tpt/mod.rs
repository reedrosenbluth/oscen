...
impl SignalProcessor for TptFilter {
    ...
    fn process<'a>(&mut self, sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        ...
        // Update parameters
        self.apply_parameter_updates(sample_rate, cutoff, q, io.f_mod);

        // Process using IO struct
        let high = (io.input - self.k * self.z[0] - self.z[1]) * self.h;
