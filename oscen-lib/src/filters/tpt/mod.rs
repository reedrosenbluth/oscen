...
impl SignalProcessor for TptFilter {
    ...
    fn process<'a>(&mut self, sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        ...
        self.f_mod = self.get_f_mod(context);

        // Get value inputs and update parameters
        let cutoff = self.get_cutoff(context);
        let q = self.get_q(context);
        self.apply_parameter_updates(sample_rate, cutoff, q, self.f_mod);

        // Call core processing logic
        TptFilter::process(self, sample_rate)
    }
}
