...
impl SignalProcessor for Oscillator {
    ...
    fn process<'a>(&mut self, sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        ...
        self.io.frequency_mod = self.get_frequency_mod(context);

        // Get value inputs from context and update node state if needed
        let phase_mod = self.get_phase(context);
        let freq_offset = self.get_frequency(context);
        let amp_mod = self.get_amplitude(context);

        if phase_mod != 0.0 {
            self.phase = (self.phase + phase_mod) % 1.0;
        }
        if freq_offset != 0.0 {
            self.frequency = freq_offset;
        }
        if amp_mod != 0.0 {
            self.amplitude *= 1.0 + amp_mod;
        }

        // Call shared processing logic
        let output = self.process_internal(sample_rate);

        // Also update the old output field for backward compat
        self.output = output;
...
impl SignalProcessor for PolyBlepOscillator {
    ...
    ///
    /// PolyBlepOscillatorIO contains { phase_mod: f32, frequency_mod: f32, output: f32 }
    fn process<'a>(&mut self, sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        // Create IO struct (generated by #[derive(Node)] macro)
        let mut io = PolyBlepOscillatorIO {
            phase_mod: self.get_phase_mod(context),
            frequency_mod: self.get_frequency_mod(context),
            output: 0.0,
        };
        ...
        // Process using struct field access - demonstrates multiple stream inputs!
        io.output = self.process_internal(
            sample_rate,
            phase_offset,
            io.phase_mod,        // Stream input from IO struct
            io.frequency_mod,    // Stream input from IO struct
            freq_input,
            amp_mod,
            pulse_mod,
        );
