...
impl crate::graph::SignalProcessor for Gain {
    fn process<'a>(&mut self, sample_rate: f32, context: &mut crate::graph::ProcessingContext<'a>) -> f32 {
        // Populate stream fields
        self.input = self.get_input(context);

        // Populate value fields
        self.gain = self.get_gain(context);

        // Call core processing logic
        Gain::process(self, sample_rate)
    }
}
