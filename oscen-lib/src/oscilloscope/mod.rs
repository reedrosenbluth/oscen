...
impl SignalProcessor for Oscilloscope {
    /// Auto-generated by macro, but we override to handle triggers and call our process().
    fn process<'a>(&mut self, sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        ...
        self.handle.push(self.input);

        // Handle triggering
        if self.get_trigger_enabled(context) > 0.5 {
            let prev = self.last_sample;
            let zero_crossing = prev <= 0.0 && self.input > 0.0;
        ...
        self.last_sample = self.input;

        // Call core processing logic
        Oscilloscope::process(self, sample_rate)
    }
}
