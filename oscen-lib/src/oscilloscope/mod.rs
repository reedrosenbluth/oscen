...
impl SignalProcessor for Oscilloscope {
    ...
    ///
    /// OscilloscopeIO contains { input: f32, output: f32 }
    fn process<'a>(&mut self, _sample_rate: f32, context: &mut ProcessingContext<'a>) -> f32 {
        ...
        io.output = io.input;
        self.handle.push(io.input);

        if self.get_trigger_enabled(context) > 0.5 {
            let prev = self.last_sample;
            let zero_crossing = prev <= 0.0 && io.input > 0.0;
